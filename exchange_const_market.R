...
# As function, can delete that tmrw
#testus <- function(S1_name, S2_name, duration, aod = "2025-06-16"){
  duration <- "1m"
  ...
  if(F){
    ...
    par(mfrow = c(1,1))
    plot(a$x, a$y, type = "l", ylim = c(0, 1.4), xlab = "30d Rolling Correlation", ylab = "Density", main = "Correlation - Transition Density")
    par(new = T)
    curve(model_plot, from = -0.99, to = 0.99, ylab = "", xlab = "", col = "red", ylim = c(0, 1.4))
    legend("topleft", c("Observed", "Fitted"), col = c("Black", "Red"), cex = 0.5, lty = 1)
  }
  
  # Test how it would look if we take THETA = 30d rolling correlation at last observation time
  # THETA <- as.numeric(tail(rolling_corr, 1))
  ...
  final_payoff <- max(test_paths[1, N+1] - test_paths[2, N+1], 0)
  ...
  plot(head(TimePoints, N), test_corr[,1], ylim = c(-1,1), main = "30d Correlation over test period", type = "l", ylab = "Rolling Correlation", xlab = "t")
...
#testus("NVDA", "AMD", "1m")
#par(mfrow = c(1,2))
#plot(training_paths[1,,24], type = "l", ylim = c(0, 200))
#par(new = T)
#plot(training_paths[2,,24], type = "l", col = "red", ylim = c(0,200))
#plot(training_paths[3,,24], type = "l")
