...
source(paste0(main_path, "scripts/functions_const.R"))

## For Saving Results
type <- "basket"
training_string <- "const"
test_string <- "const"
output_folder <- paste0(main_path, "outputs/", type, "_option/", training_string, "_", test_string, "/")
results_file <- paste0(output_folder, type, "_", training_string, "_", test_string, "_results_10.csv")
...
rhos <- c(-0.9, -0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 0.9)
for (rho in rhos) {
  ...
  
  {
    ...
    ##### Neural Networks Architecture
    {
      ...
      ###### Model Training ######
      training_paths <- path_gen_const(Ktrain) # generates training paths
      xtrain <- shape_inputs_const(arr = training_paths) # reshapes TP's
      ...
      # Test the model
      test_gens <- path_gen_const(Ktest) # Generates Ktest testpaths for S1 and S2
      xtest <- shape_inputs_const(arr = test_gens)
      ytest <- f(test_gens)
    ...
    m_mc <- mean(fin_theory <- final_payoffs - final_hedge_pos_theory)
    ...
    plot_path <- paste0(output_folder, "rho_", rho, "_hist_v", version, ".pdf")
    ...
    hist(fin_nn, main = "NN Strategy" ,ylim = c(0,2500), xlab = "Net Payoff", breaks = 20)
    ...
    # 1. Define the output path
    plot_path <- paste0(output_folder, "rho_", rho, "_hedge_over_time_v", version, ".pdf")
    ...
    # 1. Define the output path
    plot_path <- paste0(output_folder, "rho_", rho, "_pf_over_time_v", version, ".pdf")
    ...
    ## Chunk to save QQ plot as PDF
    
    # 1. Define the output path
    plot_path <- paste0(output_folder, "rho_", rho, "_qqplot_v", version, ".pdf")
    
    # 2. Open the PDF device
    pdf(plot_path)
    
    # 3. Re-run your plotting code
    qqplot(fin_nn, fin_theory, xlab = "NN Model", ylab = "MC Model", main = "Comparison final wealth NN vs. MC Model")
    ...
    qhe_mc <- mean((final_payoffs - final_hedge_pos_theory)^2)
    
    res <- data.frame(rho = rho, m_nn = m_nn,
                      m_mc = m_mc,
                      var_nn = as.numeric(var_nn),
                      var_mc = as.numeric(var_mc),
                      es_nn = es_nn,
                      es_mc = es_mc,
                      qhe_nn = qhe_nn,
                      qhe_mc = qhe_mc,
                      Ktest = Ktest)
    #save.image(paste0("/Users/martonzorenyi/Downloads/image_basket_rho", rho, "_v11.RData"))
    #print("Image saved")
